"""Train Alibaba-NLP/gte-multilingual-base as a 14-class action classifier.

"Full fine-tune + linear head + bias tuning (3ep, max_len 512, full history)"
recipe: full fine-tuning (no LoRA/frozen layers), the default single-linear
classification head (AutoModelForSequenceClassification's standard head, no
extra --head-layers), --epochs 3 and --max-length 512 are this script's
defaults already, and full (untruncated) history via qwen_serialize's
richargs/richmeta variants (max_hist=None) -- same serializer as
train_qwen3_router.py, byte-verified earlier against digital-competition's
src/data.py:serialize(). Bias tuning is a separate step: see tune_gte_bias.py
(mirrors tune_qwen3_bias.py), run after training against a held-out fold.

Mirrors train_granite_router.py / train_qwen3_router.py's `--split-mode
{group,all}` / `--oof-path` / `--exclude-ids-file` conventions. The one real
difference from those two: GTE's architecture needs trust_remote_code=True
everywhere (custom modeling code, not a stock transformers architecture).

Example:
    python scripts/train_gte_router.py \
      --data-dir ../data --output-dir ./model/gte-full-richargs-ls \
      --split-mode group --fold 0 --n-splits 5 \
      --oof-path output/oof/gte-fold0.npz

    python scripts/train_gte_router.py \
      --data-dir ../data --output-dir ./model/gte-full-richargs-ls-all \
      --split-mode all
"""
import argparse
import csv
...
def main():
    ...
    parser.add_argument("--output-dir", default="./model/gte-full-richargs-ls")
    parser.add_argument("--split-mode", choices=["group", "all"], default="group",
                        help="'group' = GroupKFold fold (has val, needed for tune_gte_bias.py "
    ...
    parser.add_argument("--save-dtype", choices=["fp32", "fp16", "bf16"], default="fp16",
                        help="fp16 default (matches the submission runtime precision; GTE is an "
                        "encoder, no ModernBert-style fp16-overflow risk seen with granite).")
