"""Class logit bias tuning for a GTE (Alibaba-NLP/gte-multilingual-base) router
checkpoint.

Same coordinate-search bias tuner as tune_granite_bias.py / tune_qwen3_bias.py,
rendering validation text with the qwen_serialize variant system (richargs
etc.). --variant/--fold/--n-splits must match whatever the checkpoint in
--model-dir was actually trained with (train_gte_router.py), or the logits
are meaningless. The one difference from tune_qwen3_bias.py: GTE needs
trust_remote_code=True to load (custom modeling code), and has no
pad-token-is-None edge case (it's an encoder tokenizer, not decoder-style).
"""
import argparse
import csv
import json
...
def build_validation_data(data_dir, fold, n_splits, variant):
...
def main():
    ...
    parser.add_argument("--model-dir", default="./model/gte-full-richargs-ls")
    ...
    parser.add_argument("--oof-path", default="",
                         help="If set, dump OOF npz (ids/y_true/classes/logits/probs) for this fold's "
                              "validation split. Lets you get OOF from an already-trained checkpoint "
                              "without retraining.")
    args = parser.parse_args()

    model_dir = Path(args.model_dir)
    ids, texts, y = build_validation_data(args.data_dir, args.fold, args.n_splits, args.variant)
    print(f"validation_samples={len(texts)} variant={args.variant}")
