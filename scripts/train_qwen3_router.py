"""Train Qwen3-Embedding-0.6B as a 14-class action classifier.

Reproduces the recipe behind `submit_0707_qwen3_ls` (full fine-tuning +
label smoothing, `richargs` serialization) with a real held-out split, since
that submission's script.py is inference-only and never recorded whether (or
which) fold it was trained on. Mirrors `train_granite_router.py`'s
`--split-mode {group,all}` / `--fold` / `--oof-path` / `--exclude-ids-file`
conventions so the two are comparable and interchangeable in downstream
scripts (find_label_issues.py, eval_search_specialist.py --base-npz).

Differences from the shipped submission on purpose:
  - No vocab-pruning / remap.npy here. That's a post-hoc, inference-only size
    optimization (see submit_0707_qwen3_ls/script.py docstring); it doesn't
    affect training or OOF validity, so it's a separate packaging step, not
    part of this script.
  - Label smoothing value is a guess (default 0.1, standard default) --
    "E8b+LS" doesn't record the exact value anywhere in the shipped zip.
    Override with --label-smoothing if you learn the real one.

Example:
    python scripts/train_qwen3_router.py \
      --data-dir ../data --output-dir ./model/qwen3-0.6b-fold0 \
      --split-mode group --fold 0 --n-splits 5 \
      --oof-path output/oof/qwen3-fold0.npz

    python scripts/train_qwen3_router.py \
      --data-dir ../data --output-dir ./model/qwen3-0.6b-all \
      --split-mode all
"""
import argparse
import csv
...
def main():
    ...
    parser.add_argument("--output-dir", default="./model/qwen3-0.6b-fold0")
    parser.add_argument("--split-mode", choices=["group", "all"], default="group",
                        ...
                        "matches the 'fold 없이 all' pattern -- can't validate locally in this mode).")
    ...
    parser.add_argument("--exclude-ids-file", default="",
                        help="Drop these ids from TRAIN only (val untouched). Same format as "
                        "train_granite_router.py: CSV with an 'id' column, or one id per line.")
    ...
    parser.add_argument("--label-smoothing", type=float, default=0.1,
                        help="Guessed default (0.1) -- the exact value used for the 'E8b+LS' "
                        "submission checkpoint isn't recorded anywhere in the shipped zip.")
    ...
    tokenizer.truncation_side = "right"  # matches submit_0707_qwen3_ls inference default
    if tokenizer.pad_token is None:  # base Qwen3 tokenizer already defines one; fallback just in case
    ...
    if args.save_dtype != "fp32" and device.type == "cuda":
        ...
        # unlike fp16 (.half()) which can craft this model's accuracy.
